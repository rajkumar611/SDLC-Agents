...
const GUARDRAILS: Guardrail[] = [
  // Input guardrails
  {
    ...
    enforcedBy: ['Requirements Agent — backend', 'Design Agent — backend', 'QA Agent — backend', 'Development Agent — backend', 'Deployment Agent — backend'],
    status: 'active',
  },
  {
    id: 'feedback-as-data',
  ...
  },
  {
    ...
    technical: 'All agent backends check stop_reason === "max_tokens" from the Anthropic API response. If triggered, a descriptive 500 error is returned so the BA can retry. The Development Agent uses max_tokens: 128000 (model maximum) to accommodate large code scaffolds.',
    enforcedBy: ['QA Agent — backend', 'Development Agent — backend', 'Deployment Agent — backend'],
    status: 'active',
  },
  {
  ...
  // Process guardrails
  {
    ...
    plainEnglish: 'The pipeline never automatically moves from one stage to the next. A designated human reviewer must explicitly click Approve before the next AI agent starts work — across all five phases: Requirements, Design, QA, Development, and Deployment. No amount of AI confidence or speed changes this.',
  ...
  },
  {
    ...
    category: 'Process Control',
    plainEnglish: 'Every pipeline run, AI output, approval, and rejection is recorded permanently across all five phases (Requirements, Design, QA, Development, Deployment). Nothing can be deleted from this log. This creates a tamper-evident paper trail for compliance and accountability.',
    technical: 'SQLite pipeline_runs stores outputs and timestamps for all five phases. pipeline_reviews records every approve/reject action. No DELETE or TRUNCATE operations exist in any route. CLAUDE.md explicitly prohibits adding them. Records are permanent by design.',
  ...
  {
    id: 'no-hardcoded-deploy-credentials',
    name: 'No Hardcoded Credentials in Deployment Config',
    category: 'Security & Compliance',
    plainEnglish: 'The Deployment Agent generates Docker configuration files but is strictly prohibited from writing real passwords, API keys, or connection strings into any file. Every sensitive value is replaced with a clearly labelled placeholder, and the developer must fill in real values separately — in a file that is never committed to version control.',
    technical: 'Deploy Agent system prompt explicitly prohibits hardcoded secrets. All generated config uses environment variable references (e.g. ${DB_PASSWORD}). .env.production.example is generated with placeholder values only. The .gitignore pattern **/.env blocks the real .env.production from being committed.',
    enforcedBy: ['Deployment Agent — system prompt', 'Deployment Agent — CLAUDE.md', '.gitignore'],
    status: 'active',
  },
  {
